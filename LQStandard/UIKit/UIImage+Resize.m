...
{
    NSLog(@"bounds%@",NSStringFromCGRect(bounds));//{{0.5, 435}, {640, 640}}
...
- (UIImage *)croppedImage:(UIImage *)image  withRect:(CGRect)rect orientation:(UIImageOrientation)orientation
{
    ...
    UIImage *clipImage = [UIImage imageWithCGImage:imageRef scale:image.scale orientation:(orientation)];//UIImageOrientationLeft
    CGImageRelease(imageRef);
    return clipImage;
}
- (UIImage *)croppedImage:(UIImage *)image  withRect:(CGRect)rect
{
    return [self croppedImage:image withRect:rect orientation:UIImageOrientationUp];
}
